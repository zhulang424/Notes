...
// NoteDAO 扩展中 DateFormatter 属性是私有的
...
+ (NoteDAO *)sharedInstance {
    static dispatch_once_t once;    // 应用声明周期中，只执行一次的代码快
    dispatch_once(&once, ^{
...
/**
 ...
 1.查询 self.plistFilePath 对应的属性列表文件是否存在
 2.存在的话结束
 3.不存在的话，从 bundle 中复制一个过去
 */
- (void)createEditableCopyOfDatabaseIfNeeded {
    ...
    if (!dbexits) {
        ...
        if (error) {
            NSAssert(success, @"错误写入文件");   // 打印错误信息并抛出异常
        }
    }

}
// 查找沙盒子目录 Documents 中 "NotesList.plist" 的文件路径
